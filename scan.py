...
def scan(path):
    ...
    for chunk in pd.read_csv(path, \
        ...
        for n, amostra in chunk.iterrows():
            if passo == 0: # Espera pela estabilização dos dados
                if n >= ign:
                    # Guarda o patamar inicial                    
                    ac = amostra
                    passo = 1
            elif passo == 1: # Verifica se a variação é maior que o threshold
                ...
                if d >= ΔP:
                    if (n - n0) >= tempo_estabilização:
                        ...
                        start_event = n - tempo_estabilização
                        passo = 3
                else:
                    passo = 1 # Volta para o passo 1 se baixou
            elif passo == 3: # Espera estabilizar
